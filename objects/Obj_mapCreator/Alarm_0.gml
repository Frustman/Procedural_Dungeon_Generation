...
var map = Obj_map.geo_map;
for(var i = 0; i < Obj_map.dungeon_width; i++){
	for(var j = 0; j < Obj_map.dungeon_height; j++){
		if(map[#i, j] == 0){
			if(i != 0 && i != Obj_map.dungeon_width - 1 && j != 0 && j != Obj_map.dungeon_height - 1){
			...
			tilemap_set_at_pixel(layer_tilemap_get_id("Tiles_2"), tile_data, i* 32, j * 32);
		}
	}
}